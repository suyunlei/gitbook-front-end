...
# TCP/IP 三次握手四次挥手
## 三次握手
1. 客户端发送syn到服务器，进入syn_sent状态，等待服务器确认
2. 服务器收到syn包，同时发送syn+ack (ack是确认包) 包，进入syn_recv状态
...
2. 用户未退出A时，打开另一个网页B
3，B收到用户请求后，返回攻击性代码，发出另一个请求访问A
4. 攻击性代码携带用户**Cookie**信息向A发出请求，导致B网站的恶意代码被执行。

**预防**
1. 添加验证码
2. 使用token
    客户端收到用户请求后，生成一个token，加密后返回给用户。
    用户需要携带这个token再次提交请求。
    服务端验证token是否正确。
